...
// It bridges a StreamEventer → UI protocol chunks with configurable options.
...
func interceptEvents(
	...
	go func() {
		...
		for ev := range eventCh {
			...
			if !opts.SendReasoning && ev.Type == StepEventReasoningDelta {
				continue
			}
			// Filter source events.
			if !opts.SendSources && ev.Type == StepEventSource {
				continue
			}
			intercepted <- ev
		}

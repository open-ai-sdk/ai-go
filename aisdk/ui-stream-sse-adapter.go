...
// Adapter translates a channel of StepEvents into UI message stream chunks.
...
func (a *Adapter) interceptEvents(
	...
	go func() {
		...
		for ev := range ch {
			...
			if state.toolCache != nil && ev.Type == StepEventToolResult && ev.ToolResult != nil {
				tr := ev.ToolResult
				state.mu.Lock()
...
func (a *Adapter) Stream(ch <-chan StepEvent, w io.Writer) string {
	state := &interceptState{}

...
func TestChunksToolCallStart_EndsActiveReasoningBlock(t *testing.T) {
	...
	chunks, _ := cp.translateEvent(StepEvent{
...
func TestChunksToolCallStart_PartOrderInPersistedBuilder(t *testing.T) {
	...
	events := []StepEvent{
		...
		{Type: StepEventReasoningDelta, ReasoningDelta: "I should call the painter."},
		{
			Type:              StepEventToolCallStart,
			ToolCallID:        "call_1",
			ToolCallName:      "painter",
		...
		{Type: StepEventToolCallReady, ToolCallID: "call_1", ToolCallName: "painter"},
		{Type: StepEventToolResult, ToolResult: &StepToolResult{

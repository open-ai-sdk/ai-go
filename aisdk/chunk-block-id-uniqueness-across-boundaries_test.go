...
func TestBlockID_TextReasoningTextHaveDistinctIDs(t *testing.T) {
	...
	events := []StepEvent{
		...
		{Type: StepEventTextDelta, TextDelta: "Hi."},
		{Type: StepEventReasoningDelta, ReasoningDelta: "Let me think."},
...
func TestBlockID_ReasoningToolReasoningHaveDistinctIDs(t *testing.T) {
	...
	events := []StepEvent{
		{Type: StepEventStepStart},
		{Type: StepEventReasoningDelta, ReasoningDelta: "Plan A."},
		{
		...
		{Type: StepEventReasoningDelta, ReasoningDelta: "Refined plan."},
	}
...
func TestBlockID_TextReasoningBoundaryClosesText(t *testing.T) {
	...
	cp := NewChunkProducer("msg-1")
	cp.translateEvent(StepEvent{Type: StepEventStepStart})
	cp.translateEvent(StepEvent{Type: StepEventTextDelta, TextDelta: "Hello."})
	textIDBefore := cp.textBlockID

	chunks := cp.chunksReasoningDelta(StepEvent{

	...
	for _, file := range files {
		ast.Inspect(file, func(node ast.Node) bool {
			...
			if strings.HasPrefix(path, "github.com/open-ai-sdk/ai-go/") &&
				...
				path != "github.com/open-ai-sdk/ai-go/uistream" {
				t.Errorf("ainode imports forbidden internal package %q", path)
			}
			first := strings.Split(path, "/")[0]
			if strings.Contains(first, ".") &&
				path != "github.com/open-ai-sdk/ai-go/aikit" &&
				path != "github.com/open-ai-sdk/ai-go/uistream" {
				t.Errorf("ainode imports non-stdlib dependency %q", path)
			}
			return false
		})
	}

...
func writeSSE(w io.Writer, c Chunk, trustedError bool) error {
	b, err := encodeChunkPayload(c, trustedError)
	if err != nil {
		return err
	}
...
func encodeChunkPayload(c Chunk, trustedError bool) ([]byte, error) {
	if !ValidChunkType(c.Type) {
		return nil, fmt.Errorf("aisdk: unknown UI message chunk type %q", c.Type)
	}

...
export async function installedUIMessageChunkTypes(): Promise<string[]> {
  ...
  const literalTypes = [
    ...source.matchAll(/\btype:\s*z\.literal\('([^']+)'\)/g),
  ].map(match => match[1]);
  ...
  if (
    ...
    throw new Error(
      `ai@7.0.35 no longer exposes the expected ${dataChunkPrefix} prefix member`,
    );
  }
  ...
  if (uniqueTypes.length !== 27) {
    throw new Error(
      `expected 27 literal UI message chunk types, found ${uniqueTypes.length}`,
    );
  }
  return uniqueTypes;
}
